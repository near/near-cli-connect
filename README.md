...
A browser-embeddable wallet connector that bridges NEAR dApps to [near-cli-rs](https://github.com/near/near-cli-rs). Instead of signing transactions in the browser, it generates CLI commands for users to run in their terminal and then verifies the resulting transactions on-chain.
...
2. When a transaction is requested, the widget renders the corresponding [near-cli-rs](https://near.cli.rs) command
